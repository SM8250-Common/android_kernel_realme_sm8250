				...
				void *data)
{
	...
	put_fs_context(fc);
#ifdef CONFIG_KSU_SUSFS_SUS_MOUNT
	if (!IS_ERR(mnt) && !READ_ONCE(susfs_is_sdcard_android_data_decrypted) && susfs_is_current_ksu_domain()) {
		int fake_mnt_id = ida_alloc_min(&susfs_mnt_id_ida, DEFAULT_KSU_MNT_ID, GFP_KERNEL);
		if (fake_mnt_id >= 0) {
			real_mount(mnt)->mnt_id = fake_mnt_id;
		}
	}
#endif // #ifdef CONFIG_KSU_SUSFS_SUS_MOUNT
	return mnt;
